...
from peewee import (JOIN, CharField, Field, ForeignKeyField, PrimaryKeyField,
                    TextField)

from .ajax import create_ajax_loader
from .form import (CustomModelConverter, InlineModelConverter, get_form,
                   save_inline)
...
class ModelView(BaseModelView):
    ...
    def _order_by(self, query, joins, sort_field, sort_desc):
        if isinstance(sort_field, string_types):
            ...
            query = query.order_by(sort_field.desc()
                                   if sort_desc else sort_field.asc())
    ...
    def create_model(self, form):
        try:
            ...
            if not self.handle_view_exception(ex):
                flash(
                    gettext(
                        'Failed to create record. %(error)s', error=str(ex)),
                    'error')
    ...
    def update_model(self, form, model):
        try:
            ...
            if not self.handle_view_exception(ex):
                flash(
                    gettext(
                        'Failed to update record. %(error)s', error=str(ex)),
                    'error')
    ...
    def delete_model(self, model):
        try:
            ...
            model.delete_instance(recursive=False)
        except Exception as ex:
            if not self.handle_view_exception(ex):
                flash(
                    gettext(
        ...
        try:
            ...
            if self.fast_mass_delete:
                ...
                for m in query:
                    ...
                    m.delete_instance(recursive=False)
                    count += 1
